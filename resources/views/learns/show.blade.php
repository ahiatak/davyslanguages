...
@section('content')
    ...
    @php $title = $learn->getTranslatedAttribute('title', $langue, 'fr') @endphp
    <br><br>
    <article id="post-185" class="post_item_single post_type_post post_format_ post-185 post type-post status-publish format-standard has-post-thumbnail hentry category-interesting-to-know category-language-skills category-research tag-research tag-skills">
        <div class="post_featured">
            <img width="2400" height="1600" src="{{Voyager::image($learn->thumbnail('cropped-2400', 'image'))}}" class="attachment-prolingua-thumb-full size-prolingua-thumb-full wp-post-image" alt="{{$learn->title}}"
                itemprop="url" srcset="{{Voyager::image($learn->thumbnail('cropped-2400', 'image'))}} 2400w, {{Voyager::image($learn->thumbnail('cropped-300', 'image'))}} 300w, {{Voyager::image($learn->thumbnail('cropped-768', 'image'))}} 768w, {{Voyager::image($learn->thumbnail('cropped-1024', 'image'))}} 1024w, {{Voyager::image($learn->thumbnail('cropped-370', 'image'))}} 370w, {{Voyager::image($learn->thumbnail('cropped-760', 'image'))}} 760w"
        ...
        <div class="post_content entry-content" itemprop="mainEntityOfPage">
            ...
            {!! $learn->getTranslatedAttribute('description', $langue, 'fr') !!}
            ...
            <div class="post_meta post_meta_single"><span class="post_meta_item post_tags"><a
                        href="{{route('learns.index')}}" rel="tag">DAVYS'LANGUAGES</a></span><span class="post_meta_item post_share">
                    <div
                        class="socials_share socials_size_tiny socials_type_block socials_dir_horizontal socials_wrap">
                        <span class="social_items">
                            <a
                                href="https://twitter.com/intent/tweet?text={{ $learn->getTranslatedAttribute('title', $langue, 'fr') }}&#038;url=http%3A%2F%2Fdavyslanguages.com%2F2017%2F08%2F12%2F{{$langue}}%2Fposts%2F{{$learn->id}}%2F"
                                class="social_item social_item_popup"
                                data-link="https://twitter.com/intent/tweet?text={{ $learn->getTranslatedAttribute('title', $langue, 'fr') }}&#038;url=http%3A%2F%2Fdavyslanguages.com%2F2017%2F08%2F12%2F{{$langue}}%2Fposts%2F{{$learn->id}}%2F"
                            ...
                            </a><a href="http://www.facebook.com/sharer.php?u=http%3A%2F%2Fdavyslanguages.com%2F2017%2F08%2F12%2F{{$langue}}%2Fposts%2F{{$learn->id}}%2F" class="social_item social_item_popup" data-link="http://www.facebook.com/sharer.php?u=http%3A%2F%2Fdavyslanguages.com%2F2017%2F08%2F12%2F{{$langue}}%2Fposts%2F{{$learn->id}}%2F"
